...
- **Faza B (SIMULĂRI) — 1/13 DONE (pilot), 12 rămase**:
  ...
  - **[ ] RĂMAS Faza B — 12 lucrări**: 2022 (model, simulare, var-01, var-03), 2023 (model, simulare, var-01, var-06, var-07), 2024 (var-03, var-09, var-10). **Rețetă (pilot dovedit)**: render subiect p1(0.45–0.93)+p2 + barem p1(0.66–0.96)+p2 zoom 3.0–3.2 → transcrie SII+SIII verbatim (matrice inline `(r/r/r)`, legi compoziție, analiză) + rubric a/b/c din barem → adaugă obiect în `PAPERS[]` din `import-exam-bac-matematica-m1-batch.mjs` → `--validate` (90p check) → agent math re-verify II+III → git commit/push → VPS2 backup+pull+import → DB query confirm. SI reutilizează cerințele originale din grile (dar ca SHORT+finalAnswer, nu MCQ).
