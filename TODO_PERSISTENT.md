...
- [x] **W6 — grilele celor 8 module** — DONE 2026-09-06: **62 de grile** (ciorne) prin Claude CLI, după 5 defecte de calitate găsite prin măsurare, nu prin presupunere: prompt care cerea markdown în enunț (`838f809`) · parse totul-sau-nimic (`766da03`) · întrebări care nu stăteau singure + `onlyEmpty` care îngheța module (`797833e`) · **scurgerea prin lungime** 71%→37% (`43860e6`, L34) · **scurgerea prin poziție** 44%→29% cu amestecare deterministă (`f3cd040`, L35). Raport `Reports/GRILE-agent-imobiliar-2026-09-06.md`. Copie a ordinii vechi: `VPS2:/root/backups/agent-imobiliar-optiuni-pre-shuffle-2026-09-06.json`.
- [x] **W6 — cursul PUBLICAT pentru agenți** — 2026-09-08. 8 lecții + 56 de grile, toate publicate; fiecare modul are lecția și testul lui (7/7/4/9/8/7/8/6). Cod de acces emis: **WSVZ-CRHW**, 25 de folosiri, expiră 07.10.2026.
  - Decizie user: publicăm fără verificare juridică prealabilă, corecțiile vin prin feedback negativ. Înainte de publicare am trecut totuși cele 8 lecții printr-o verificare de fapte (68 de constatări ridicate, **3 confirmate** după verificare adversarială, toate reparate): golul de finanțare calculat ca diferența întreagă de evaluare (agentul recomanda o tăiere de preț de 4.000 € pe o cifră inventată) · pragul de afișări dat ca regulă fermă · onorariul notarial trecut pe lista „monedelor care nu costă bani” (art. 1666 alin. (1) Cod civil). Copie de siguranță: `VPS2:/root/backups/lectii-agent-imobiliar-pre-corectii-2026-09-08.json`.
  - Trei goluri prinse de proba de la capăt la capăt, nu de citit codul: nu exista rută de publicare a unui curs (`a78d185`) · `/dashboard/lessons` era în afara oricărui meniu, deci lecțiile publicate n-aveau ușă (`b0a9351`) · poarta de abonament rula înainte de a ști despre ce materie e vorba, deci un agent înscris de firmă primea 403 „face parte dintr-un pachet” (`9ac59ad`).
  - Verificat pe producție ca agent real: neînscris → 404 pe tot · intră cu codul → rol STUDENT · vede materia cu 56 de grile · deschide lecția (6.386 caractere) · pornește testul (15 întrebări).
- [x] **W7 — testezi doar din materia parcursă** — DONE + LIVE 2026-09-08 (`5874365`). Cerință user, cu analogia elevului de clasa a VIII-a: un modul intră în test doar după ce lecția lui e terminată. Regulă pură (`src/lib/course-topics.ts`, 11 teste), citită din `LessonProgress` — pe care cititorul de lecții îl scrie singur, deci nu cere bifare manuală ca la materiile școlare. Verificat live: fără nicio lecție terminată → „Termină o lecție ca să se deschidă testul modulului ei"; după lecția 1 → 7 întrebări, doar din „Fundamentele meseriei".
- [x] **W8 — limba lecțiilor, curățată în trei treceri** — DONE 2026-09-08. Userul a semnalat „mandat" (calc după *mandate*), pe care prima verificare automată îl ratase. (1) verificare pe categorii: 86 ridicate / 39 confirmate; (2) **măturare obiectivă** — fiecare cuvânt care arată englezește, scos automat cu context, fără ca ceva să decidă singur ce contează → `prescoring` folosit de 3 ori și explicat de 0 ori, `listing` în 4 locuri; (3) **citire ca începător** (om de 34 de ani venit din vânzări de piese auto, notează unde se împiedică) → 80 ridicate / **43 confirmate, toate aplicate** în 34 de editări (4 dintre ele fuseseră deja aplicate într-o rundă anterioară; 39 noi). Cele mai grele: `antecontract` și `cartea funciară` explicate la prima folosire (erau explicate abia în modulul 7) · cheltuielile de la notar defalcate, cu cine le plătește după lege · data limită a clauzei de finanțare calculată din calendarul lecției, iar scriptul corectat (spunea o dată care contrazicea regula) · „arvuna nu e un avans" · de unde iei lista de anunțuri expirate · ce faci la prima prezentare dacă n-ai încă dosare proprii. `mandat` ca anglicism = **0**. Raport reexportat: `Reports/CURS-agent-imobiliar-2026-09-08.md`. Lecție **L38**.
  - Găsite de verificarea de după editări, nu de agenți: o **frază dublată** în modulul 5 (înlocuire pusă peste o ancoră parțială, dintr-o rundă anterioară) — de aceea editările se ancorează acum pe linia luată din baza de date, nu pe text retastat, iar la final rulează un detector de blocuri repetate; și **cele 4 copii de siguranță ale lecțiilor erau JSON invalid** (dublu-escapate), deci nerestaurabile programatic — reparate toate (`/root/backups/lectii-*.reparat.json`) + una nouă validă după corecturi (`lectii-post-cititor-2026-09-08.json`).
- [x] **Scurgerile distribuționale, restul fețelor** — VERIFICATE EMPIRIC 2026-09-06 (`7e21f08` + `27dc5dc` + `2a78b68`). Nu erau două fețe, erau cinci. Măsurat pe lotul de 62: familia sintactică **57%** (virgule; caracterele erau curate la +1%, indiciul se mutase în sintaxă) · acord gramatical **45%** · rezolvabil fără enunț **95-100%** (șase atacuri oarbe, patru au ales identic pe toate 62) · distractori recognoscibil falși (agentul cu criterii pur formale: **62/62**) · **aceeași întrebare cu chei diferite** (ruta trimitea doar numărul grilelor existente, nu textul). Reparat la trei altitudini: promptul (cauza), `blind-check.ts` (proprietatea, nu indiciile), `guess-baseline.ts` (familia, nu unitatea) + `near-duplicate.ts`. Raport: `Reports/SCURGERI-GRILE-2026-09-06.md`. Lecție L36.
- [x] **Verificat pe lotul regenerat 2026-09-06** (56 de grile din 67 generate): ghicitorul de suprafață **50% → 19%** (sub linia întâmplării de 25%); atacul „varianta care iese din tipar” 95% → 27%; diferența de propoziții +23% → +2%, de virgule +47% → +10%. **Scurgerea de suprafață e ÎNCHISĂ.**
- [ ] **Rezolvarea oarbă semantică: 97% → 52-61% — înjumătățită, nu eliminată.** Semnalul rămas, în cuvintele atacatorilor: „varianta care exprimă practica recomandată de curs”. Limita e onestă — într-o materie care predă practica onestă, răspunsul corect *va fi* cel onest, iar cine îl alege a învățat, nu a trișat. Nu se poate separa fără **elevi reali**; a face valorile cursului ambigue ca să scadă cifra ar fi pedagogie mai proastă. Următorul pas nu e încă un filtru.
- [ ] **Fețele încă nemăsurate**: ordinea alfabetică a variantelor, lungimea explicației, corelarea SEMANTICĂ între itemi. Detectorul lexical prinde doar 9-10%, neschimbat între loturi — ipoteza că regula „distractori adevărați care răspund la altă întrebare” ar fi creat corelarea a fost **respinsă de măsurătoare**, deci n-am livrat gardă pentru ea.
- [ ] **Re-măsoară după fiecare lot nou**: `npx tsx scripts/measure-guess-baseline.ts <slug-materie>`. De trei ori la rând am declarat clasa închisă după ce am reparat unitatea pe care o măsuram.
- [x] **REZOLVAT 2026-09-09** (`9154110`) — rutele pe `domainId` răspund acum 404 ca cele pe slug, prin `resolveDomainByIdOrForbid`. Descoperit că aceeași lipsă bloca și superadminul din propriile materii private. Original: `resolveDomainOrForbid` (`src/lib/domain-gate.ts`) întoarce corect 404 pentru cine nu e înscris, dar lucrează pe **slug**; `/api/student/lessons?domainId=...` are propria verificare și răspunde **403** (măsurat pe producție 2026-09-08, materiile Poșta: pe slug → 404 ca o materie inexistentă, pe id → 403). Exploatarea cere un cuid, care nu se ghicește și nu apare nicăieri pentru un neînscris, deci secretul practic ține — dar cele două căi ar trebui să răspundă la fel. De aliniat la 404, cu test care le compară.
- [x] **REPARAT 2026-09-24 (`c82019e`) — Poarta de modul vede DOAR primul curs al unei materii.** `courseTopicsFor` (`src/lib/course-topics.ts`) face `findFirst` pe cursurile publicate ale materiei, deci într-o materie cu două sau mai multe cursuri publicate, testele celui de-al doilea nu se deschid niciodată — omul termină lecțiile și banca rămâne goală, fără nicio eroare. Găsit 2026-09-08 la planificarea demo-ului Poșta (unde am ales, tocmai de-aia, câte o materie per rol). Reparația e mică (`findMany` + reunirea modulelor), dar atinge o poartă live: merită sesiune proprie, cu test care pune două cursuri în aceeași materie.
...
- [~] 🎨 **Redesign UX-proactiv (aprobат de user 2026-06-29 pe mockup deck, cu condiția: dashboard-ul clasic rămâne opțiune)** — F1–F4 DONE + LIVE etutor.ro; F5 = sesiune dedicată:
  - **[x] F1 „Astăzi"** (`91f44af`): home-ul elevului = o acțiune-erou a zilei (din zona slabă / recommendation) + drum ghidat 3 pași + serie + 2 statistici; **dashboard-ul clasic intact** sub toggle „Astăzi / Panou complet" (persistat localStorage, default Astăzi). Doar date deja returnate de API.
  - **[x] F2 Onboarding de învățare** (`b22f9e4`): elevul nou (0 înscrieri) primește wizard pe home: alege materia (1 tap, enroll) → test de calibrare (quick session) → pasul 3 vine natural din „Astăzi". Lista completă de domenii rămâne accesibilă din wizard.
  - **[x] F3 Soft-lock contextual** (`0c7cbf6`): `FeatureLockCard` conduce cu problema elevului („Greșești des la «topic» (N%)") + valoarea pachetului pe exact acea zonă + **punte gratuită** („Continuă gratuit cu grile") — lacătul nu mai e fundătură. Fallback generic fără zonă slabă; API-ul rămâne gate-ul real.
  - **[x] F4 Sinteză + triaj** (`46e8099`): PĂRINTE — card narativ pe regulă în capitolul copilului („X a studiat 3 din 7 zile 🔥… se împiedică de «topic»") + acțiune (memento pe topic → tab Remindere); MEDITATOR — „N elevi au nevoie de atenție azi" pe hub (top 3 după `failureProbability`≥60, factor principal + link profil). Zero „AI" în copy.
  - **[ ] F5 Baterie de teste remediale (piesa forte)** — AI generează set de grile exact pe punctele slabe; depinde de tag-uirea fină la import („cos 30°", „discriminant"). Sesiune mare dedicată. Închide bucla self-guiding + devine cârligul premium (F3) + valoarea părintelui (F4).
  - Mockup deck aprobат: artifact „eTutor — Audit UX & propunere proactivă" (sesiunea 2026-06-29). Verificate live per fază: login real + pagini 200 + chunk-uri cu textele noi + pm2 fără crash-loop.
...
> ✅ **STRAT UMAMI (cookieless) — DONE + LIVE 2026-07-03** (commits `7a434bb`+`20b545b`, deploy VPS2, sesiune mesh). websiteId `7cc3296f-5e08-49ad-adca-b161dc7400d0` provisionat în Umami central; `<UmamiScript>` în root layout folosind **`analytics.knowbest.ro`** (aliasul neutru = default-ul lib-ului `UMAMI_DEFAULT_SRC`; nota „DNS pending" era stale — domeniul e LIVE cu SSL, → VPS1). CSP `src/middleware.ts` += `analytics.knowbest.ro` pe script-src+connect-src (lecția techbiz: CSP strict = zero date). vendored tgz `file:vendor/analytics.tgz` + `transpilePackages`. tsc curat (0 erori din schimbări; 1 eroare pre-existentă în exam-engine.test = neatinsă). Verificare onestă: tag în HTML servit = `analytics.knowbest.ro` + CSP permite (0 refs techbiz) + script.js 200 + POST /api/send 200 (eveniment acceptat pt websiteId). **Rămas de confirmat manual**: o vizită browser real (UA normal) → hit vizibil în dashboard Umami (curl/headless sunt filtrate de Umami). **Notă corecție**: prima variantă (`7a434bb`) rutase greșit prin `analytics.techbiz.ae` (ecosistem techbiz) — corectat la `analytics.knowbest.ro` (ecosistem knowbest, corect pt etutor.ro) în `20b545b`.
>
