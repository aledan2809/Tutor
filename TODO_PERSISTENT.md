...
- **🟡 ÎN CURS — 20 teste de antrenament CNCE** (`pro-matematica2`: 2021 Test_02..15 = 14 + 2022 Test_01..06 = 6). **Perechi separate** Test_NN.pdf (subiect) + Barem_NN.pdf (barem) — ca seria 2, playbook se aplică direct. Slug `test-NN`, source „EN VIII <an> Testul N — antrenament (CNPEE)". Figuri = **imagini raster** (IMG xref, bbox exact via fig_inspect — fără risc clip vectorial). NB: pagina = A4 595×842 (rulează fig_inspect/figură). Decizie 2026-06-04: figuri = **fitz local** (4uPDF extract-figure = client-side draw-box, fără backend; premium feature „planned"=neconstruit). Itemii OPEN: sub-prompt-urile a)/b) incluse în `content` (whitespace-pre-line) — mai complet decât seria 2.
  ...
  - **🔴 RĂMASE (18)**: 2021 Test_04..15 (12) + 2022 Test_01..06 (6). Rețetă identică: copiază `scripts/import-exam-mate-2021-test-03.mjs` → transcrie din Test_NN+Barem_NN → validate → fig_inspect bbox-uri → **figuri via 4uPDF /api/extract-region** (venv `/tmp/4updf-venv` păstrat) → montage-verify → commit → git pull VPS → import pe prod DB → pm2 restart → verificare autentificată (score + figuri 200). **Total prod acum: 28 papers / 504 items.**
