...
- [ ] **Poarta de modul vede DOAR primul curs al unei materii.** `courseTopicsFor` (`src/lib/course-topics.ts`) face `findFirst` pe cursurile publicate ale materiei, deci într-o materie cu două sau mai multe cursuri publicate, testele celui de-al doilea nu se deschid niciodată — omul termină lecțiile și banca rămâne goală, fără nicio eroare. Găsit 2026-09-08 la planificarea demo-ului Poșta (unde am ales, tocmai de-aia, câte o materie per rol). Reparația e mică (`findMany` + reunirea modulelor), dar atinge o poartă live: merită sesiune proprie, cu test care pune două cursuri în aceeași materie.
