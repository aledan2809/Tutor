...
- **Total exam-bank prod acum: 40 papers / 720 items** (2021 Test_02..15 complet importate 2026-06-04; rămân 2022 Test_01..06).
- **🟡 ÎN CURS — 20 teste de antrenament CNCE** (`pro-matematica2`: 2021 Test_02..15 = 14 ✅ DONE + 2022 Test_01..06 = 6 rămase). **Perechi separate** Test_NN.pdf (subiect) + Barem_NN.pdf (barem) — ca seria 2, playbook se aplică direct. Slug `test-NN`, source „EN VIII <an> Testul N — antrenament (CNPEE)". Figuri = **imagini raster** (IMG xref, bbox exact via fig_inspect — fără risc clip vectorial). NB: pagina = A4 595×842 (rulează fig_inspect/figură). Decizie 2026-06-04: figuri = **fitz local** (4uPDF extract-figure = client-side draw-box, fără backend; premium feature „planned"=neconstruit). Itemii OPEN: sub-prompt-urile a)/b) incluse în `content` (whitespace-pre-line) — mai complet decât seria 2.
  ...
  - **🔴 RĂMASE (6)**: 2022 Test_01..06. Rețetă identică (cu **/review mesh** pe fiecare script): copiază un `scripts/import-exam-mate-2021-test-NN.mjs` → transcrie din Test_NN+Barem_NN → validate → fig_inspect → **figuri via 4uPDF /api/extract-region** (venv `/tmp/4updf-venv` + token `/tmp/figtoken.txt` păstrate; backend pe :8099) → montage-verify → commit → git pull VPS → import prod DB → pm2 restart → verificare autentificată → `/review`. **Total prod acum: 40 papers / 720 items** (2021 Test_02..15 complet).
