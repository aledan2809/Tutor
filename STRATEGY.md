...
- [x] **Demo interactiv above-the-fold + social proof** — LIVE 2026-06-01 (commit `7517b1b`). Magic Quiz embeddat în hero (sample pre-încărcat, refolosit din `<MagicQuizDemo variant=hero|full>`) + social proof REAL din DB (1.400+ grile via `unstable_cache`, counter vizitatori self-revealing la ≥50) + trust signals verificabile + how-it-works 3 pași + footer/nav. Verificat live (RO/EN 200, demo generează 5 grile). `/try` refactorizat să refolosească același component.
- [ ] **Screenshot-uri reale + video 15s** — owner-pending (necesită assets reale; NU fabricate). Slot de cod marcat în `src/app/[locale]/page.tsx` secțiunea „What you get". Demo-ul live acoperă deja rolul de preview.
- [ ] **Testimoniale** — owner-pending (necesită testimoniale reale; nu se inventează). De adăugat ca secțiune când există conținut real.
- [x] **CTA WhatsApp-first** — LIVE 2026-06-02 (commit `93b26b5`). `<WhatsAppCta>` floating site-wide (via `[locale]/layout`), RO/EN, **env-gated `NEXT_PUBLIC_SUPPORT_WHATSAPP`** — apare doar când owner-ul setează un număr real (NU se afișează niciun număr fals). Bucla de share WhatsApp era deja LIVE (demo/duel/certificat/referrals); asta adaugă canalul de contact „întreabă înainte de signup".
...
**Tier 4 — Fricțiune zero la intrare:** ✅ **majoritar LIVE/prezent 2026-06-02 (commit `93b26b5`)**
