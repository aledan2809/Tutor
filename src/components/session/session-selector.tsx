...
export function SessionSelector({
  ...
  const reasonText = (reason: string) => {
    if (reason === "Regular practice") return t("reasons.regular");
    let m: RegExpMatchArray | null;
    if ((m = reason.match(/^(\d+) days since last session$/))) return t("reasons.daysSince", { n: m[1] });
    if ((m = reason.match(/^Exam in (\d+) days$/))) return t("reasons.examIn", { n: m[1] });
    if ((m = reason.match(/^(\d+) weak area/))) return t("reasons.weakAreas", { n: m[1] });
    return reason;
  };
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border-2 border-blue-600 bg-blue-600/5 p-6">
        ...
        <div className="mb-4 flex gap-4 text-sm text-gray-500">
          ...
          <span>{recommended.questionCount} {t("questions")}</span>
        </div>
        <button
