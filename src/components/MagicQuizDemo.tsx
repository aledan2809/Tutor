...
// Shared interactive Magic Quiz demo. Used both as the above-the-fold hero
// widget on the homepage (variant="hero") and as the full standalone page body
// at /try (variant="full"). Single source of truth for the generate → take →
// score → share/CTA loop; both surfaces call the same public API endpoints.
...
export default function MagicQuizDemo({
  ...
  const T = ro
    ? {
        ...
        generate: "Începe testul ✨",
        generating: "Pregătesc întrebările…",
    ...
    : {
        ...
        fullDemo: "See the full demo →",
      };

  // On the hero variant we pre-load the sample so the visitor is one click from
  // the WOW moment without having to find or paste material first.
  const [text, setText] = useState(hero ? (ro ? SAMPLE_RO : SAMPLE_EN) : "");
    ...
    <div className={hero ? "w-full text-left" : ""}>
      {phase === "input" && (
        <>
          ...
          <textarea
            ...
            rows={hero ? 5 : 9}
            maxLength={MAX_CHARS}
            className={`${hero ? "mt-0" : "mt-6"} w-full rounded-xl border border-gray-700 bg-gray-900 p-4 text-sm text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500`}
          />
          <div className="mt-2 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">{text.length} / {MAX_CHARS}</span>
          </div>
      ...
      {hero && phase === "input" && (
        <div className="mt-3 text-center">
          <Link href="/try" className="text-sm text-blue-400 hover:text-blue-300">
            {T.fullDemo}
          </Link>
        </div>
      )}
